...
The `_doh` group is **inactive by default**. It contains common DNS-over-HTTPS endpoints (`dns.google`, `cloudflare-dns.com`, `mozilla.cloudflare-dns.com`, etc.). Flip its rule to `"is_active": true` if you have a browser configured to use a specific DoH provider and want to force it back to system DNS so this daemon can see (and block) its queries. In `strict` mode, `_doh` is intentionally enforced at the DNS layer only — IP-blocking those endpoints in pf would also block the daemon's own use of `1.1.1.1:53` as `backup_dns`.
